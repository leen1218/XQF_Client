...
class LoginViewController: UIViewController, UITextFieldDelegate, RequestHandler
{
	...
	@IBAction func login(_ sender: UIButton) {
		...
		guard self.cellphone.text != "" && self.password.text != "" else {
			showAlert(title: "登录失败", message: "用户名或者密码不能为空!", parentVC: self, okAction: nil)
			return
		}
	...
	{
		let request:XQFRequest = XQFRequestManager.shared().createRequest(ENUM_REQUEST_LOGIN)
	...
	func onSuccess(_ response: Any!) {
		...
		if (result_json["status"] != nil) {
			if (result_json["status"] as! String == "200") {
				...
				showAlert(title: "密码不正确", message:"密码不正确，请重新输入！", parentVC: self, okAction: nil)
			} else {
	...
	func onFailure(_ error: Error!) {
		showAlert(title: "请求失败", message: "登录请求失败，请重试!", parentVC: self, okAction: nil)
	}
