...
class RegisterViewController: UIViewController, UITextFieldDelegate, RequestHandler
{
	...
	@IBAction func register(_ sender: UIButton) {
		...
		guard self.password.text! == self.confirmPassword.text! else {
			showAlert(title: "密码不一致", message: "您两次输入的密码不一致，请确认！", parentVC: self, okAction: nil)
			return
		}
		
		// 2. 检测code是否为空
		guard self.authCode.text != nil else {
			showAlert(title: "验证码错误", message: "验证码不能为空!", parentVC: self, okAction: nil)
			return
		}
		
		// 3. 向服务器发送注册请求
		let request:XQFRequest = XQFRequestManager.shared().createRequest(ENUM_REQUEST_REGISTER)
	...
	@IBAction func getAuthCode(_ sender: UIButton) {
		...
		guard self.cellPhone.text != nil else {
			showAlert(title: "手机号错误", message: "手机号码不能为空!", parentVC: self, okAction: nil)
			return
		}
		
		// 2. 向服务器发送注册码请求
		let request:XQFRequest = XQFRequestManager.shared().createRequest(ENUM_REQUEST_AUTHENTICATION_CODE)
	...
	func onSuccess(_ response: Any!) {
		...
		if (result_json != nil) {
			if (result_json?["msg"] != nil) {
				...
				showAlert(title: "请求返回", message: msg!, parentVC: self, okAction: nil)
			} else {
				showAlert(title: "请求失败", message:"请重新发送", parentVC: self, okAction: nil)
			}
		}
	}
	func onFailure(_ error: Error!) {
		showAlert(title: "请求失败", message: "网络请求失败，请重试!", parentVC: self, okAction: nil)
	}
